...
>
  ...
  >
    ...
    <svg
      id="save.svg"
      viewBox="0 0 24 24"
      class="svg-btn-nf {has_unsaved_changes ? 'fill-yellow-500' : 'fill-slate-300'}"
  ...
  >
    {#each entrys as it (it.id)}
      <div class="flex" animate:flip={{ duration: 200 }}>
        <div class="w-8 fixed flex justify-center items-center bg-red-500">
          <svg
            id="remove.svg"
            viewBox="0 0 24 24"
            class="w-8 h-full fill-white scale-90 hover:scale-100 cursor-pointer"
        ...
        <div
          class="w-full z-10 transition duration-150 ease-out {edit_mode
            ? 'translate-x-8'
            : ''}"
        >
